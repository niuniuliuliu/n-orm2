...
class BaseEntity {
    ...
    instance(obj) {
        return new this.orm.entitys[this.entityKey](obj);
    }
    ...
    [buildQuerySql](entity, relations, queryCondition) {
        ...
        let join = (relation, tableName) => {
            let E = this.orm.entitys[relation.entityKey];
        ...
        if (queryCondition.conditions && queryCondition.conditions.length > 0)
            sql += 'and ' + buildCondition(queryCondition.conditions).join(' and ');
        if (queryCondition.orConditions && queryCondition.orConditions.length > 0)
            sql += ' and (' + buildCondition(queryCondition.orConditions).join(' or ') + ')';
            ...
            for (let relation of otmRelations) {
                ...
                if (otmArr && Array.isArray(otmArr)) {
                    for (let i = 0; i < otmArr.length; i++) {
                        ...
                        let e = new this.orm.entitys[relation.entityKey](item);
                        otmArr[i] = e;
    ...
    async [update](entity, cascade) {
        ...
        let sql = `update ${entity.tableName} set ${updateColumns.join(',')} where 1=1 ${conditions.join(' and ')}`;
        await this.db.execute(sql, dbParams);
        return entity;
    }
    ...
    async [del](entity, cascade) {
        ...
        let sql = `delete from ${entity.tableName} where 1=1 ${conditions.join(' and ')}`;
        await this.db.execute(sql, dbParams);
        return entity;
    }
}
